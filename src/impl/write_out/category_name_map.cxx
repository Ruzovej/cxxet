...
category_name_map::register_category_name(output::category_flag const category,
                                          std::string &&name,
  ...
  if (category == output::category_flag_none) {
    for (unsigned idx = 0; idx < max_user_categories; ++idx) {
      ...
      if (past_the_end || names[idx].empty()) {
        ...
        return output::category_flag{1u << idx};
      }
    }
...
std::string_view category_name_map::get_joined_category_names(
    output::category_flag const category_bits) const {
  ...
  SUBCASE("is_one_bit_set") {
    SUBCASE("all valid cases") {
      for (unsigned i{0u}; i < 32u; ++i) {
        REQUIRE_EQ(output::category_flag{1u << i}.is_one_bit_set(), true);
      }
    }

    SUBCASE("some invalid cases") {
      REQUIRE_EQ(output::category_flag{0u}.is_one_bit_set(), false);
      REQUIRE_EQ(output::category_flag{3u}.is_one_bit_set(), false);
      REQUIRE_EQ(output::category_flag{5u}.is_one_bit_set(), false);
      REQUIRE_EQ(output::category_flag{6u}.is_one_bit_set(), false);
      REQUIRE_EQ(output::category_flag{7u}.is_one_bit_set(), false);
      REQUIRE_EQ(output::category_flag{1u | 2u}.is_one_bit_set(), false);
      REQUIRE_EQ(output::category_flag{128u | 4069u}.is_one_bit_set(), false);
      REQUIRE_EQ(output::category_flag{0xFF'FF'FF'FFu}.is_one_bit_set(), false);
    }
  }

  SUBCASE("bit_to_index") {
    for (unsigned i{0u}; i < 32u; ++i) {
      REQUIRE_EQ(output::category_flag{1u << i}.bit_to_index(), i);
    }
  }
...
TEST_CASE("write_out::category_name_map itself") {
  ...
  SUBCASE("optimistic cases") {
    SUBCASE("no registration") {
      ...
      REQUIRE_EQ(cnm.get_joined_category_names(output::category_flag{1u << 30}),
                 "cxxet_1");
      REQUIRE_EQ(cnm.get_joined_category_names(output::category_flag{1u << 31}),
                 "cxxet_2");
      REQUIRE_EQ(cnm.get_joined_category_names(
                     output::category_flag{1u << 30 | 1u << 31}),
                 "cxxet_1,cxxet_2");
    }

    SUBCASE("single registration") {
      auto const category{cnm.register_category_name(output::category_flag_none,
                                                     "test_category", false)};

      REQUIRE_EQ(category, output::category_flag{1u});
    ...
    SUBCASE("multiple registrations") {
      auto const cat1{
          cnm.register_category_name(output::category_flag_none, "c1", false)};
      auto const cat2{
          cnm.register_category_name(output::category_flag_none, "c2", false)};
      auto const cat3{cnm.register_category_name(
          output::category_flag{1u << 15}, "c3", false)};

      REQUIRE_EQ(cat1, output::category_flag{1u});
      REQUIRE_EQ(cat2, output::category_flag{2u});
      REQUIRE_EQ(cat3, output::category_flag{1u << 15});
    ...
    SUBCASE("auto-assign exhaustion") {
      for (unsigned i{0u}; i < category_name_map::max_user_categories; ++i) {
        auto const cat{cnm.register_category_name(
            output::category_flag_none, "Cat-" + std::to_string(i), false)};
        REQUIRE_EQ(cat, output::category_flag{1u << i});
      }

      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag_none, "one_too_many", false),
                        std::runtime_error);
      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag_none,
                            "one_too_many_regardless_of_rename", true),
                        std::runtime_error);

      REQUIRE_EQ(cnm.get_joined_category_names(output::category_flag_none), "");
      REQUIRE_EQ(
          cnm.get_joined_category_names(output::category_flag{0x3F'FF'FF'FFu}),
      ...
      REQUIRE_EQ(
          cnm.get_joined_category_names(output::category_flag{0xFF'FF'FF'FFu}),
    ...
    SUBCASE("valid single bit") {
      auto const category{cnm.register_category_name(output::category_flag{8u},
                                                     "test_category", false)};
      REQUIRE_EQ(category, output::category_flag{8u});

      REQUIRE_EQ(cnm.get_joined_category_names(output::category_flag{4u}), "");
      REQUIRE_EQ(cnm.get_joined_category_names(output::category_flag{16u}), "");
      REQUIRE_EQ(cnm.get_joined_category_names(output::category_flag{4u | 16u}),
                 "");

      REQUIRE_EQ(
          cnm.get_joined_category_names(output::category_flag{4u | 8u | 16u}),
          "test_category");
      REQUIRE_EQ(
          cnm.get_joined_category_names(output::category_flag{0x3F'FF'FF'FFu}),
          "test_category");
    ...
    SUBCASE("valid single bit - different positions") {
      auto const cat1{
          cnm.register_category_name(output::category_flag{1u}, "cat1", false)};
      auto const cat2{
          cnm.register_category_name(output::category_flag{4u}, "cat2", false)};
      auto const cat3{cnm.register_category_name(output::category_flag{16u},
                                                 "cat3", false)};

      REQUIRE_EQ(cat1, output::category_flag{1u});
      REQUIRE_EQ(cat2, output::category_flag{4u});
      REQUIRE_EQ(cat3, output::category_flag{16u});
    }

    SUBCASE("allow rename") {
      auto const cat1{cnm.register_category_name(output::category_flag{2u},
                                                 "original_name", false)};
      REQUIRE_EQ(cat1, output::category_flag{2u});
      REQUIRE_EQ(cnm.get_joined_category_names(cat1), "original_name");

      auto const cat2{cnm.register_category_name(output::category_flag{2u},
                                                 "new_name", true)};
      REQUIRE_EQ(cat2, output::category_flag{2u});
      REQUIRE_EQ(
          cnm.get_joined_category_names(output::category_flag{0x3F'FF'FF'FF}),
          "new_name");
    }
  }
  ...
  SUBCASE("error cases") {
    SUBCASE("invalid names") {
      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag_none, "", false),
                        std::runtime_error);
      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag_none, " invalid", false),
                        std::runtime_error);
      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag_none, "1invalid", false),
                        std::runtime_error);
      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag_none, "-invalid", false),
                        std::runtime_error);
      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag_none, "_invalid", false),
                        std::runtime_error);
      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag_none, "invalid name", false),
                        std::runtime_error);
      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag_none, "invalid!", false),
                        std::runtime_error);
    ...
    SUBCASE("used name") {
      (void)cnm.register_category_name(output::category_flag_none,
                                       "some_name_1", false);

      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag_none, "some_name_1", false),
                        std::runtime_error);
      ...
      (void)cnm.register_category_name(output::category_flag_none,
                                       "some_name_3", false);
      auto const cat4{cnm.register_category_name(output::category_flag_none,
                                                 "some_name_4", false)};

      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag_none, "some_name_2", false),
                        std::runtime_error);
      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag_none, "some_name_3", true),
                        std::runtime_error);
      REQUIRE_THROWS_AS(
    ...
    SUBCASE("category with multiple bits set") {
      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag{1u | 2u}, "invalid", false),
                        std::runtime_error);

      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag{2u | 4u}, "invalid", false),
                        std::runtime_error);

      REQUIRE_THROWS_AS(
          (void)cnm.register_category_name(output::category_flag{1u | 2u | 4u},
                                           "invalid", false),
          std::runtime_error);
    }
    ...
    SUBCASE("category using reserved bits") {
      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag{
                                1u << category_name_map::max_user_categories},
                            "reserved1", false),
                        std::runtime_error);
    ...
    SUBCASE("duplicate registration without allow_rename") {
      auto const cat1{cnm.register_category_name(output::category_flag{4u},
                                                 "original", false)};
      REQUIRE_EQ(cat1, output::category_flag{4u});

      REQUIRE_THROWS_AS((void)cnm.register_category_name(
                            output::category_flag{4u}, "duplicate", false),
                        std::runtime_error);
    }
  }
}
