...
void flush_thread() noexcept { impl::thread_local_sink::instance()->flush_to_global(); }
...
void marker::append_record(long long const start_ns,
  ...
  impl::thread_local_sink::instance()->append_record({desc, color, tag, start_ns, end_ns});
}
