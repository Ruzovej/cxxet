...
function setup_file() {
    ...
    export CXXET_EXAMPLE_EXECUTABLE="${CXXET_TMP_BUILD_DIR}/cxxet_fetch_content_direct_usage_example"
...
@test "Check build properties described in generated 'compile_commands.json'" {
    ...
    assert_equal "$(jq -e -c "[ .[] | select(.directory == \"${CXXET_TMP_BUILD_DIR}\") ] | map(.file) | unique | sort" "${compile_commands}")" "[\"${CXXET_ROOT_DIR}/examples/cmake_fetch_content/direct_usage/direct_usage.cxx\"]"
